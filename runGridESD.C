...
AliAnalysisGrid* CreateAlienHandler(TString mode="test",Int_t sub=0,TString fname="testName")
{
	...
	for(Int_t irun = 75; irun < 78; irun++)
	{
	    Printf("%d %d",irun,runnumbers[irun]);
	    plugin->AddRunNumber(runnumbers[irun]);
	}
    ...
    plugin->SetAdditionalLibs("Ali_AS_Event_V3.h Ali_AS_EventLinkDef.h Ali_DarkMatter_ESD_analysis.h Ali_DarkMatter_ESD_analysis.cxx vertex_modified.h");
